...
*"/ps2/CHD 1/"*) core="pcee2_libretro"; src="https://archive.org/download/PS2-part1-CHD-Arquivista/";;
*"/ps2/CHD 2/"*) core="pcee2_libretro"; src="https://archive.org/download/PS2-part2-CHD-Arquivista/";;
*"/ps2/CHD 3/"*) core="pcee2_libretro"; src="https://archive.org/download/PS2-part3-CHD-Arquivista/";;
*"/ps2/NonRedump A-G/"*) core="pcee2_libretro"; ext="iso"; src="https://archive.org/download/non-redump_sony_playstation2_part1/";;
*"/ps2/NonRedump H-Q/"*) core="pcee2_libretro"; ext="iso"; src="https://archive.org/download/non-redump_sony_playstation2_part2/";;
*"/ps2/NonRedump R-Z/"*) core="pcee2_libretro"; ext="iso"; src="https://archive.org/download/non-redump_sony_playstation2_part3/";;
...
if [[ "$core" == "mame_libretro" || "$core" == "mame_libretro "* ]]; then
  ...
  if [[ -d /usr/share/games/mame/hash ]]; then
    hashpath_opt=" -hashpath /usr/share/games/mame/hash"
  elif [[ "$src" =~ /mame-sl/([^/]+)\.zip/ ]]; then
    ...
    if [[ -f "$sysdir/mame/hash/$list.xml" ]] || curl -sfL -o "$sysdir/mame/hash/$list.xml" \
      ...
      hashpath_opt=" -hashpath \"$sysdir/mame/hash\""
    else
      rm -f "$sysdir/mame/hash/$list.xml"
    fi
  fi
  ...
  # MAME game fails. The ROM directory is readable by definition -- the core has
  # to load the ROM out of it -- and it lands in MAME's rompath for free.
  cmd_file="$(dirname "$1")/.gameflix-$$.cmd"
  # -rp, not -rompath: the core appends a "-rp" path to its own rompath and then
  # drops the switch, so RetroArch's system/mame/bios and system/mame/roms stay
  # searchable. A literal -rompath reaches MAME last and replaces them.
  echo "${mame_args:+$mame_args }$rom${rompath:+ -rp \"$rompath\"}$hashpath_opt -skip_gameinfo -snapname \"$basename\"" > "$cmd_file"
