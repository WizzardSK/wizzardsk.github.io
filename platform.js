...
function generateFileLinks(romPath, imagePath) {
    ...
    // upravenie cesty podľa protokolu
    if (location.protocol !== "file:") {
        if (romPath.includes("2600")) { 
            romPath = romPath.replace("myrient", "https://myrient.erista.me/files"); 
        } else if (romPath.startsWith("myrient/")) { 
            romPath = romPath.replace("myrient", "https://myrient.erista.me/files"); 
        }
    } else { 
        romPath = "../" + romPath; 
    ...
    fileNames.forEach(fileName => {
        ...
        // encodeURI nechá lomky nezmenené, zakóduje medzery a špeciálne znaky
